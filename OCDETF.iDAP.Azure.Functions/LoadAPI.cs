    ...
    public static class LoadAPI
    {
            ...
            ILogger log)
        {
            ...
            try
            {
                ...
                {
                    createFileName = string.Format("FILE-" + DateTime.Now.ToString("yyyyMMdd HHmmss").ToUpper()) + ".download";
            ...
            string responseMessage = string.IsNullOrEmpty(errorMessage)
                ? $"Hello, This HTTP triggered function executed successfully created {createFileName} from {data.apiURL}, {data.appName}, {data.category} {Path.Combine(Path.GetTempPath(), createFileName)}!!"
                : errorMessage;
